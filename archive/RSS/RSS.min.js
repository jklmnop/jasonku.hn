...
RiseVision.RSS.Controller.prototype.getMediaDimensions=function(a,b,c){var d={},e;$("#hiddenContainer").empty();$("#hiddenContainer").append($(".item").clone());"2x1-headline.xml"==prefs.getString("layout")||"2x1-no-headline.xml"==prefs.getString("layout")?(d.width=0.5*prefs.getString("rsW"),d.height=prefs.getString("rsH")/this.itemsCount-$("#hiddenContainer .textWrapper").eq(a).outerHeight(!0)-2*this.mediaPadding,0==$("#hiddenContainer .story").eq(a).length&&"2x1-headline.xml"==prefs.getString("layout")&&
(d.width=prefs.getString("rsW")-2*this.mediaPadding)):"1x2-headline.xml"==prefs.getString("layout")||"1x2-no-headline.xml"==prefs.getString("layout")?(d.width=prefs.getString("rsW")-2*this.mediaPadding,d.height=(prefs.getString("rsH")/this.itemsCount-2*this.mediaPadding)/2):"4x1-headline.xml"==prefs.getString("layout")||"4x1-no-headline.xml"==prefs.getString("layout")?(d.width=0.33*prefs.getString("rsW"),d.height=prefs.getString("rsH")/this.itemsCount-2*this.mediaPadding):"background.xml"==prefs.getString("layout")&&
...
RiseVision.RSS.Controller.prototype.fadeOut=function(){var a=this;this.isFadingIn||this.isFadingOut||(this.showFeedTimer=null,this.isFadingOut=!0,$("#container").fadeTo("slow",0.01,function(){a.isFadingOut=!1;a.currentItemIndex=a.nextItemIndex;a.isPaused||a.showItem()}))};RiseVision.RSS.Controller.prototype.startTimer=function(){var a=this;clearTimeout(this.showFeedTimer);this.showFeedTimer=setTimeout(function(){a.showNext()},this.transitionHold);this.isPaused||(this.startTimeout=new Date)};
RiseVision.RSS.Controller.prototype.pauseTimer=function(){var a,b=!1;this.isPlaying&&(clearTimeout(this.showFeedTimer),clearTimeout(this.resumeVideoTimer),$(".videoWrapper").each(function(a){$(this).is(":visible")&&(jwplayer(a).play(!1),b=!0)}),!b&&$(".item:first").is(":visible")&&(this.timeLeft=this.transitionHold,this.timeLeft-=new Date-this.startTimeout,a=Math.round(this.timeLeft/1E3),this.timeLeft=1E3*a,this.isDone||20<=prefs.getInt("transitionHold")&&10>=a))&&(this.timeLeft=0,this.isDone?this.isFeedLoaded?
this.showItem():this.loadFeed():this.showNext())};RiseVision.RSS.Controller.prototype.resumeTimer=function(){var a=this;this.hasVideo(this.currentItemIndex)?(this.showItem(),jwplayer(0).play(!0)):this.isLoading?(this.timeLeft=this.transitionHold,this.isFeedLoaded?this.showItem():this.loadFeed()):(this.isDone&&(this.timeLeft=this.transitionHold,this.isDone=!1),clearTimeout(this.showFeedTimer),this.showFeedTimer=setTimeout(function(){a.showNext()},this.timeLeft))};
